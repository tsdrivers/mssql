...
export function resolveLibraryPath(ctx: ResolutionContext): string {
  ...
  // 1. Explicit env var
  const envPath = ctx.env("TSDRIVERS_MSSQL_LIB_PATH");
  if (envPath && ctx.fileExists(envPath)) return envPath;

  // 2. node_modules location (unified package or runtime-specific)
  const nodeModulesUnified =
    `${ctx.cwd()}${sep}node_modules${sep}@tsdrivers${sep}mssql${sep}native${sep}${filename}`;
  if (ctx.fileExists(nodeModulesUnified)) return nodeModulesUnified;
  const nodeModulesRuntime =
    `${ctx.cwd()}${sep}node_modules${sep}@tsdrivers${sep}mssql-${ctx.info.runtime}${sep}native${sep}${filename}`;
  ...
  // 6. Cache directory
  const cachePath =
    `${home}${sep}.cache${sep}@tsdrivers${sep}mssql${sep}${ctx.version}${sep}${filename}`;
  ...
  throw new Error(
    `Native library "${filename}" not found.\n` +
      ...
      "\n" +
      "Or set TSDRIVERS_MSSQL_LIB_PATH to the library path.",
  );
}
